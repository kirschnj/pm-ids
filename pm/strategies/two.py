...
class InternalDuelingGame(Game):
    ...
    def get_d(self):
        return self.original_game.get_d()
    ...
    def get_actions(self, indices):
        ...
        """
        first = [i[0] for i in indices]
        second = [i[1] for i in indices]

        actions = (self.original_game.get_actions(first) + self.original_game.get_actions(second))/2
        return actions
    ...
    def get_observation_maps(self, indices):
        ...
        first = [i[0] for i in indices]
        second = [i[1] for i in indices]
        return (self.original_game.get_observation_maps(first) - self.original_game.get_observation_maps(second))/2
